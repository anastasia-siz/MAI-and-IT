...
def geo_to_gcrs(lat, lon, height=0.0, jd=None, time_str=None):
    """
    Преобразует географические координаты (широта, долгота, высота) в систему ГЦСК (GCRS).
    ...
    height : float, опционально
        Высота над уровнем моря в км (по умолчанию 0.0)
    jd : float, опционально
        Юлианская дата (если не указана, используется time_str)
    time_str : str, опционально
        Строка времени в формате ISO (например, '2023-01-01T12:00:00')

    Возвращает:
    ------------
    pos_gcrs : numpy.ndarray
        Массив [x, y, z] координат в системе ГЦСК в км
    """
    # Проверка входных данных
    if not (-90 <= lat <= 90):
        raise ValueError("Широта должна быть в диапазоне [-90, 90] градусов")
    if not (-180 <= lon <= 180):
        raise ValueError("Долгота должна быть в диапазоне [-180, 180] градусов")
    ...
    if jd is not None:
        ...
        raise ValueError("Необходимо указать либо jd, либо time_str")

    # Создание местоположения на Земле
    ...
    # Преобразование в ITRS (Международная земная система отсчёта)
    itrs_coord = location.get_itrs(obstime=t)

    # Преобразование из ITRS в GCRS (ГЦСК)
    gcrs_coord = itrs_coord.transform_to(GCRS(obstime=t))

    # Извлечение координат в км
    pos_gcrs = gcrs_coord.cartesian.xyz.to(u.km).value
...
if __name__ == "__main__":
    ...
    moscow_lat = 55.7558
    moscow_lon = 37.6173
    moscow_height = 0.15  # 150 м = 0.15 км

    # Время — 1 января 2023, 12:00 UTC
    jd_example = 2459945.0  # Юлианская дата для 2023-01-01 12:00

    # Преобразование координат
    moscow_gcrs = geo_to_gcrs(
        lat=moscow_lat,
        lon=moscow_lon,
        height=moscow_height,
        jd=jd_example
    )

    print("Координаты Москвы в системе ГЦСК (км):")
    print(f"X: {moscow_gcrs[0]:.3f} км")
    print(f"Y: {moscow_gcrs[1]:.3f} км")
    print(f"Z: {moscow_gcrs[2]:.3f} км")
